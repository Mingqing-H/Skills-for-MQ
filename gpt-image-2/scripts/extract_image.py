...
def find_image_blobs(session_paths: list[pathlib.Path]) -> list[tuple[str, str]]:
    """Return unique image payloads found across given files, largest first."""
            ...
            for match in BASE64_BLOB_PATTERN.finditer(flat):
                ...
                for magic, ext in IMAGE_MAGIC_PREFIXES.items():
                    if blob.startswith(magic):
                        if blob not in seen:
                            seen.add(blob)
                            found.append((blob, ext, len(blob)))
                        break
    found.sort(key=lambda item: item[2], reverse=True)
    return [(blob, ext) for blob, ext, _size in found]
...
def find_best_image_blob(session_paths: list[pathlib.Path]) -> tuple[str, str] | None:
    ...
    blobs = find_image_blobs(session_paths)
    if not blobs:
        return None
    return blobs[0]
...
def main(argv: list[str]) -> int:
    ...
    session_paths = [
        pathlib.Path(line)
        for line in sessions_list_path.read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
